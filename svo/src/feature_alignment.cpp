...
// µü´ú¼ÆËãÌØÕ÷µãÔÚÐÂÍ¼ÏñÖÐµÄÎ»ÖÃ£¬¹âÁ÷·¨
//(u',v') =  argmin_uv (||I(u',v') - Affine* I(u,v) ||)^2   from the svo paper
// r = I(u',v') - Affine*I(u,v)
// ÓÃµü´ú·¨ÇóÕâ¸ö×îÐ¡¶þ³ËÎÊÌâ, deltaÎªµü´úÔöÁ¿:
// J^T*J*delta = -J*r    -->    H*delta = -J*r
// J ÊÇ¸÷¸öÏñËØµãµÄÑÅ¿Ë±È¾ØÕóJi ÀÛ¼ÆÆðÀ´µÄÒ»¸ö¾ØÕó, J = [ J1;J2;...Ji...Jn], n = patch_size
// Ji = [ dr/du' , dr/dv'] = [ dI(u',v')/du' , dI(u',v')/dv']  ¾ÍÊÇI'ÔÚx,yÁ½¸ö·½ÏòµÄµ¼Êý
bool align2D(
    const cv::Mat& cur_img,
    uint8_t* ref_patch_with_border,
    uint8_t* ref_patch,
    ...
    bool no_simd)
{
  ...
  float __attribute__((__aligned__(16))) ref_patch_dy[patch_area_]; // ¶¨ÒåÒ»¸ö16×Ö½Ú¶ÔÆëµÄ±äÁ¿
  Matrix3f H; H.setZero();

  // compute gradient and hessian
  // Ji = [dx,dy,1]    // ×îºóµÄ1 ÊÇ¶Ô²Ð²î½øÐÐÇóµ¼£¬dr/r , ËûÕâÀï»¹¶Ôpatch µÄ²Ð²î½øÐÐÁË¹À¼Æ.
  ...
  for(int y=0; y<patch_size_; ++y)
  {
    ...
    {
      Vector3f J;
      J[0] = 0.5 * (it[1] - it[-1]); // dx
      J[1] = 0.5 * (it[ref_step] - it[-ref_step]); // dy
      J[2] = 1;
      *it_dx = J[0];
      *it_dy = J[1];
      H += J*J.transpose(); // H =J^T * J = sum( [ dxdx, dxdy,  dx
                            //                          dydx, dydy, dy
                            //                          dx   ,  dy   ,  1]  )
    }
  }
  ...
  // Compute pixel location in new image:
  float u = cur_px_estimate.x();   // ÌØÕ÷¿éÖÐÐÄÏñËØµãÔÚÐÂÍ¼ÏñÖÐµÄÎ»ÖÃ
  float v = cur_px_estimate.y();
...
//    float new_chi2 = 0.0;
    ...
    for(int y=0; y<patch_size_; ++y)
    {
      ...
      {
        float search_pixel = wTL*it[0] + wTR*it[1] + wBL*it[cur_step] + wBR*it[cur_step+1]; // ÔÚcurÖÐµÄÏñËØÖµ£¬Ë«ÏßÐÔ²åÖµ
        float res = search_pixel - *it_ref + mean_diff; // cur ºÍref ÖÐÏñËØ²îÖµ¡£¼ÓÉÏmean_diff  å
